...
// Mevcut TravianTaskQueue kodları ...

// TravianTaskQueue içine eklenecek yeni fonksiyonlar ve UI ekleme

(function() {
    'use strict';

    // Görev penceresine ekleyeceğimiz inputlar ve buton
    function addRepeatControls() {
        const taskWindow = document.querySelector('#taskWindow');
        if (!taskWindow) return;
        if (document.querySelector('#repeatCountInput')) return; // Zaten eklenmişse çık

        const waitLabel = document.createElement('label');
        waitLabel.textContent = 'Bekleme Süresi (dk): ';
        const waitInput = document.createElement('input');
        waitInput.type = 'number';
        waitInput.min = 1;
        waitInput.value = 3;
        waitInput.id = 'waitTimeInput';

        const repeatLabel = document.createElement('label');
        repeatLabel.textContent = 'Tekrar Sayısı: ';
        const repeatInput = document.createElement('input');
        repeatInput.type = 'number';
        repeatInput.min = 1;
        repeatInput.value = 200;
        repeatInput.id = 'repeatCountInput';

        const repeatButton = document.createElement('button');
        repeatButton.textContent = 'Tekrarla';
        repeatButton.id = 'repeatButton';

        const container = document.createElement('div');
        container.style.marginTop = '10px';
        container.appendChild(waitLabel);
        ...
        taskWindow.appendChild(container);

        repeatButton.addEventListener('click', () => {
            const waitTime = parseInt(waitInput.value, 10) * 60000;
            const repeatCount = parseInt(repeatInput.value, 10);

            if (isNaN(waitTime) || waitTime < 60000) {
                alert('Lütfen 1 dakikadan büyük geçerli bekleme süresi girin.');
                return;
            }
            ...
            addTasksRepeatedly(currentTask, repeatCount, waitTime);
        });
    }

    // Örnek: Görev bilgisini al (burayı oyuna göre düzenle)
    function getCurrentTaskData() {
        // TODO: Görev penceresindeki gerçek bilgileri çek
        return {
            targetVillageId: '12345',
            troops: { spear: 100, sword: 50 },
            taskType: 'raid'
        };
    }

    // Görevleri sırayla ekle (burayı oyun scriptine göre ayarla)
    async function addTasksRepeatedly(taskData, count, interval) {
        for (let i = 0; i < count; i++) {
            console.log(`Görev ekleniyor ${i+1}/${count}`, taskData);
            await addTaskToQueue(taskData);
            await sleep(interval);
        }
        alert('Tüm görevler eklendi!');
    }

    function addTaskToQueue(taskData) {
        return new Promise(resolve => {
            // Buraya görev ekleme kodunu koy
            console.log('Görev eklendi', taskData);
    ...
    // Görev penceresi açıldıkça kontrol et, inputları ekle
